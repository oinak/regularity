...
/* PRIVATE Helpers */
  ...
  function interpret(){
    var flat = [].concat.apply([], arguments);
    switch (flat.length) {
      case 2:
...
/* PUBLIC OBJECT API: */
  ...
  this.startWith = function() {
    if (empty()) {
      return this.write('^%s', interpret(arguments));
    } else {
  ...
  this.then = this.append = function(pattern){
    return this.write(interpret(args));
  };

  // The line must end with the specified pattern
  this.endWith = function(pattern){
    return this.write('%s$', pattern);
  };
  ...
  this.oneOf = function(){
    var flat = [].concat.apply([], args),
        values = flat.map(function(c){
          return escapeStr(c);
        }).join('|');
    return this.write('[%s]',values);
  };
