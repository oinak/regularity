...
var Regularity = function() {
    ...
    var interpret = function(){
        ...
        switch (arguments.length) {
            ...
            default:
                throw new Error('interpret: wrong number of arguments: got ' + arguments.length + ', expected 1 or 2');
        }
    }

    // Ex: (2, 'x') or (3, 'digits')
    var numberedConstraint = function(count, type){
        //console.log('    numberedConstraint('+count+', '+type+')');
        return _makeRepeatedPattern(patternedConstraint(type), count);
    }

    // Ex: ('aa') or ('$')
    var patternedConstraint = function(pattern){
    ...
    //   escapeStr("one.two")
    //   # => "one\.two"
    var escapeStr = function(pattern){
    ...
    var singularizeWord = function(word){
...
/* PUBLIC OBJECT API: */
    ...
    this.startWith = function() {
        if (!empty()) throw new Error('can\'t call startWith more than once!');
    ...
    this.between = function(range, pattern){
        if (!isValidRange(range)) throw new Error("between: you must provide an array of 2 elements, at least one of them must be an integer");

        var beginning = range[0] || '0';
        var end       = range[1] || '0';

        return write(_makeRangedPattern(interpret(pattern), beginning, end));
    };
