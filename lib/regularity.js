...
var Regularity = function() {
    ...
    var _makeRangedPattern = function(pattern, beginning, end) {
        return (group.nonCapturing(pattern) + '{' + beginning + ',' + end + '}');
    };

    var _makeRepeatedPattern = function(pattern, count) {
        return ((count === 1) ?
                pattern       :
                (group.nonCapturing(pattern) + '{' + count + '}'));
    };
    ...
    this.maybe = function() {
        _appendPatternChunk(quantify.zeroOrOne(group.nonCapturing('%s')), _interpret(arguments));
    ...
    this.oneOf = function() {
        ...
        _appendPatternChunk(group.nonCapturing('%s'), choices);
    ...
    this.zeroOrMore = function() {
        _appendPatternChunk(quantify.zeroOrMore(group.nonCapturing('%s')), _interpret(arguments));

        return this;
    };

    this.oneOrMore = function() {
        _appendPatternChunk(quantify.oneOrMore(group.nonCapturing('%s')), _interpret(arguments));
