...
var Regularity = function() {
  ...
  function escapeStr(pattern){
    ...
    for (var i in pattern) {
      if ( _escapedChars.indexOf( pattern[i] ) !== -1 ){
        escaped += '\\' + pattern[i];
      } else {
        escaped += pattern[i];
      }
...
/* PUBLIC OBJECT API: */
  ...
  this.between = function(range, pattern){
    ...
    if (isArray(range) &&
      ...
      replaced = '%s{%f,%t}'.replace('%s', values[0]).replace('%f', values[1]).replace('%t', values[2]);
  ...
  this.insensitive = function(){
    if (_options.indexOf('i') == -1) {
      ...
      var twiceGlobalError = new Error('insensitive() called twice');
      throw(twiceGlobalError);
    }
    return this;
  };
  ...
  this.global = function(){
    if (_options.indexOf('g') == -1) {
      ...
      var twiceGlobalError = new Error('global() called twice');
      throw(twiceGlobalError);
    }
    return this;
  };
  ...
  this.multiLine = function(){
    if (_options.indexOf('m') == -1) {
      ...
      var twiceGlobalError = new Error('multiLine() called twice');
      throw(twiceGlobalError);
    }
    return this;
  };
  ...
  this.done = function(){
    return new RegExp( _result, _options);
  };
